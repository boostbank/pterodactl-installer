    ...
    async runCommand(args) {
        if (commands.hasCommand(args)) {
            ...
            try {
                ...
                console.title("Setting up Web Server...");
                const domain = flags.hasFlag("domain") ? flags.getFlag("domain") : "none";
                if(domain === "none"){
                    console.printError("Must have domain passed to this command. Ex: sudo pti webserver -domain=sub.test.com");
                    process.exit(1);
                }else{
                    console.title(`Using domain: ${domain}`)
                    await sh("apt install -y certbot");
